...
RegisterNetEvent("huijari:munapaa")
AddEventHandler("huijari:munapaa", function()
	huijariii()
end)
...
next_ped = function(drugToSell)
	...
	cops = 0
	ESX.TriggerServerCallback('jdev_huumemyynti:Hankikyttatieto', function(_cops)
		cops = _cops
	end)
			...
			if distance < 10.0 then
				...
				if IsControlJustPressed(0, 38) and canSell then
					...
					if reject <= 3 then
						...
						TriggerServerEvent('jdev_huumemyynti:ilmoituspoliiseille', drugToSell)
						SetPedAsNoLongerNeeded(npc.ped)
					...
					SetPedAsNoLongerNeeded(npc.ped)
					TriggerServerEvent('jdev_huumemyynti:pay', drugToSell)
...
RegisterNetEvent('jdev_huumemyynti:findClient')
AddEventHandler('jdev_huumemyynti:findClient', next_ped)

RegisterNetEvent('jdev_huumemyynti:notifyPolice')
AddEventHandler('jdev_huumemyynti:notifyPolice', function(coords)	
