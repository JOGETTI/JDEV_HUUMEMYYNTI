...
AddEventHandler('myy:huume2', function()
    ...
    for k, v in pairs(Config.drugs) do
        ...
        if drugToSell.count ~= 0 then
            ...
            TriggerClientEvent('jdev_huumemyynti:findClient', source, drugToSell)
            break
        end
...
RegisterServerEvent('jdev_huumemyynti:pay')
AddEventHandler('jdev_huumemyynti:pay', function(drugToSell)
    xPlayer = ESX.GetPlayerFromId(source)
	...
	if checkki > 0 then
		...
		TriggerClientEvent('esx:showNotification', xPlayer.source, ('~r~Mikä vitun scämmeri sä oot?????')) -- Notification for exploiter / cheater
		TriggerClientEvent('huijari:munapaa', source)
	end
end)

RegisterServerEvent('jdev_huumemyynti:ilmoituspoliiseille')
AddEventHandler('jdev_huumemyynti:ilmoituspoliiseille', function(drugToSell)
    TriggerClientEvent('jdev_huumemyynti:notifyPolice', -1, drugToSell.coords)
end)

ESX.RegisterServerCallback('jdev_huumemyynti:Hankikyttatieto', function(source, cb)
    count = 0
